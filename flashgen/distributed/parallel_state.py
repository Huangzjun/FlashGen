...
from flashgen.distributed.device_communicators.base_device_communicator import (DeviceCommunicatorBase)
from flashgen.distributed.utils import StatelessProcessGroup
...
class GroupCoordinator:
    ...
    def __init__(
        ...
        if use_device_communicator and self.world_size > 1:
            ...
            if not current_platform.is_npu_alike():
                raise RuntimeError("Flashgen is NPU-only: device communicator requires NPU.")
            from flashgen.distributed.device_communicators.npu_communicator import (NpuCommunicator)
        ...
        # self.use_custom_op_call = current_platform.is_npu_alike()
...
def init_distributed_environment(
    ...
    from flashgen.platforms import current_platform
    if not current_platform.is_npu_alike():
        raise RuntimeError("Flashgen is NPU-only: distributed backend requires NPU.")
    backend = "hccl"
