...
"""The arguments of Flashgen Inference."""
import argparse
import dataclasses
import json
...
from flashgen.configs.configs import PreprocessConfig
from flashgen.configs.pipelines.base import PipelineConfig
from flashgen.configs.utils import clean_cli_args
from flashgen.layers.quantization import QUANTIZATION_METHODS, QuantizationMethods
from flashgen.logger import init_logger
from flashgen.utils import FlexibleArgumentParser, StoreBoolean
...
class FlashgenArgs:
    ...
    # the Flashgen-internal precedent).
    ...
    # Typed transformer quantization carrier. The typed inference API resolves
    # ``engine.quantization.transformer_quant`` to a concrete
    # ``QuantizationConfig`` instance, then pins it on
    # ``pipeline_config.dit_config.quant_config``. ``None`` leaves whatever
    # value the caller already set on ``dit_config`` untouched.
    ...
    def _apply_transformer_quant(self) -> None:
        ...
        a caller writes ``engine.quantization.transformer_quant`` in their
        config. We pin it here rather than at request time so
    ...
    def add_cli_args(parser: FlexibleArgumentParser) -> FlexibleArgumentParser:
        ...
        parser.add_argument(
            ...
            default=FlashgenArgs.mode.value,
            help="The mode to run Flashgen",
        )
        ...
        parser.add_argument(
            ...
            default=FlashgenArgs.workload_type.value,
        ...
        parser.add_argument(
            ...
            type=str,
            choices=["mp"],
            default=FlashgenArgs.distributed_executor_backend,
        ...
        parser.add_argument(
            "--inference-mode",
            action=StoreBoolean,
            default=FlashgenArgs.inference_mode,
        ...
        parser.add_argument(
            "--trust-remote-code",
            action=StoreBoolean,
            default=FlashgenArgs.trust_remote_code,
            help="Trust remote code when loading HuggingFace models",
        )
        parser.add_argument(
            "--revision",
            type=str,
            default=FlashgenArgs.revision,
        ...
        parser.add_argument(
            "--num-gpus",
            type=int,
            default=FlashgenArgs.num_gpus,
            help="The number of GPUs to use.",
        )
        parser.add_argument(
            "--tp-size",
            type=int,
            default=FlashgenArgs.tp_size,
            help="The tensor parallelism size.",
        )
        parser.add_argument(
            "--sp-size",
            type=int,
            default=FlashgenArgs.sp_size,
            help="The sequence parallelism size.",
        )
        parser.add_argument(
            "--hsdp-replicate-dim",
            type=int,
            default=FlashgenArgs.hsdp_replicate_dim,
            help="The data parallelism size.",
        )
        parser.add_argument(
            "--hsdp-shard-dim",
            type=int,
            default=FlashgenArgs.hsdp_shard_dim,
            help="The data parallelism shards.",
        )
        parser.add_argument(
            "--dist-timeout",
            type=int,
            default=FlashgenArgs.dist_timeout,
        ...
        parser.add_argument(
            "--output-type",
            type=str,
            default=FlashgenArgs.output_type,
            choices=["pil"],
        ...
        parser.add_argument(
            "--prompt-txt",
            type=str,
            default=FlashgenArgs.prompt_txt,
        ...
        parser.add_argument(
            ...
            nargs="+",
            type=str,
            default=FlashgenArgs.lora_target_modules,
        ...
        parser.add_argument(
            ...
            action=StoreBoolean,
            default=FlashgenArgs.enable_torch_compile,
        ...
        parser.add_argument(
            "--master-port",
            type=int,
            default=FlashgenArgs.master_port,
        ...
        parser.add_argument(
            ...
            action=StoreBoolean,
            default=FlashgenArgs.enable_stage_verification,
            help="Enable input/output verification for pipeline stages",
        )
        parser.add_argument(
            "--override-text-encoder-safetensors",
            type=str,
            default=FlashgenArgs.override_text_encoder_safetensors,
        ...
        parser.add_argument(
            ...
            default=FlashgenArgs.override_text_encoder_quant,
            help="Quantization method for text encoder override",
        )
        parser.add_argument(
            "--override-transformer-cls-name",
            type=str,
            default=FlashgenArgs.override_transformer_cls_name,
            help="Override transformer cls name",
        )
        parser.add_argument(
            "--override-pipeline-cls-name",
            type=str,
            default=FlashgenArgs.override_pipeline_cls_name,
    ...
    @classmethod
    def from_cli_args(cls, args: argparse.Namespace) -> "FlashgenArgs":
        ...
        for attr in attrs:
            if attr == 'pipeline_config':
                ...
                # Convert string to ExecutionMode enum
                mode_value = getattr(args, attr, FlashgenArgs.mode.value)
                ...
                # Convert string to WorkloadType enum
                workload_type_value = getattr(args, 'workload_type', FlashgenArgs.workload_type.value)
    ...
    @classmethod
    def from_kwargs(cls, **kwargs: Any) -> "FlashgenArgs":
        ...
        # Filter to only FlashgenArgs dataclass fields — pipeline-specific CLI
        # args (e.g. enable_bsa, bsa_sparsity) live in PipelineConfig and must
        # not be forwarded to the FlashgenArgs constructor.
    ...
    def check_fastvideo_args(self) -> None:
        ...
        if self.pipeline_config is None:
            raise ValueError("pipeline_config is not set in FlashgenArgs")
        ...
        if self.mode == ExecutionMode.PREPROCESS:
            if self.preprocess_config is None:
                raise ValueError("preprocess_config is not set in FlashgenArgs when mode is PREPROCESS")
...
@dataclasses.dataclass
class TrainingArgs(FlashgenArgs):
    """
    Training arguments. Inherits from FlashgenArgs and adds training-specific
    ...
    negative_prompt_embeds_path: str = ""
    ...
    def add_cli_args(parser: FlexibleArgumentParser) -> FlexibleArgumentParser:
        ...
        parser.add_argument(
            ...
            type=str,
            default="",
            help=("Directory containing pre-encoded negative prompt tensors "
                  "(negative_prompt_embeds.pt, negative_prompt_attention_mask.pt)"),
        )
