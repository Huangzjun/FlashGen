# `flashgen/models/` — Model Implementations
...
1. Read `flashgen/layers/AGENTS.md` first — it defines which tensor-parallel
...
- Adding training-only state (EMA buffers, optimizer state) to the inference
  state-dict surface.
- Calling `torch.distributed` directly. Go through `flashgen.distributed`.
