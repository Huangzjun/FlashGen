...
"""Logging configuration for flashgen."""
import datetime
import json
import logging
import os
...
FLASHGEN_CONFIGURE_LOGGING = envs.FLASHGEN_CONFIGURE_LOGGING
FLASHGEN_LOGGING_CONFIG_PATH = envs.FLASHGEN_LOGGING_CONFIG_PATH
FLASHGEN_LOGGING_LEVEL = envs.FLASHGEN_LOGGING_LEVEL
FLASHGEN_LOGGING_PREFIX = envs.FLASHGEN_LOGGING_PREFIX
...
_FORMAT = (f"{FLASHGEN_LOGGING_PREFIX}%(levelname)s %(asctime)s.%(msecs)03d "
...
def _configure_fastvideo_root_logger() -> None:
    ...
    if not FLASHGEN_CONFIGURE_LOGGING and FLASHGEN_LOGGING_CONFIG_PATH:
        raise RuntimeError("FLASHGEN_CONFIGURE_LOGGING evaluated to false, but "
                           "FLASHGEN_LOGGING_CONFIG_PATH was given. FLASHGEN_LOGGING_CONFIG_PATH "
                           "implies FLASHGEN_CONFIGURE_LOGGING. Please enable "
                           "FLASHGEN_CONFIGURE_LOGGING or unset FLASHGEN_LOGGING_CONFIG_PATH.")

    if FLASHGEN_CONFIGURE_LOGGING:
        logging_config = DEFAULT_LOGGING_CONFIG

    if FLASHGEN_LOGGING_CONFIG_PATH:
        if not path.exists(FLASHGEN_LOGGING_CONFIG_PATH):
            raise RuntimeError("Could not load logging config. File does not exist: %s", FLASHGEN_LOGGING_CONFIG_PATH)
        with open(FLASHGEN_LOGGING_CONFIG_PATH, encoding="utf-8") as file:
    ...
    for formatter in logging_config.get("formatters", {}).values():
        ...
        if formatter.get("class") == "flashgen.logging.NewLineFormatter":
            formatter["class"] = "flashgen.logging_utils.NewLineFormatter"
...
def enable_trace_function_call(log_file_path: str, root_dir: str | None = None):
    ...
    """
    logger.warning("FLASHGEN_TRACE_FUNCTION is enabled. It will record every"
