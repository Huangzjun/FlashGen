# `flashgen/configs/` — Config-Driven Model Registry
...
`flashgen/registry.py` imports every concrete `PipelineConfig` and exposes
...
3. Add the import + name mapping in `flashgen/registry.py`.
...
- Importing from `flashgen.pipelines` here. Configs are the lower layer; the dependency is one-way.
