# `flashgen/pipelines/` — Pipeline Composition
...
- Stages must be **deterministic given the same `ForwardBatch + FlashgenArgs`**. Side effects (logging, profiling) only.
- Read all knobs from the passed-in `FlashgenArgs` / `PipelineConfig`. Never `os.getenv` directly.
