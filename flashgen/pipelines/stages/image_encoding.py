...
from flashgen.fastvideo_args import ExecutionMode, FlashgenArgs
from flashgen.forward_context import set_forward_context
from flashgen.logger import init_logger
from flashgen.models.vaes.common import ParallelTiledVAE
from flashgen.models.vision_utils import (get_default_height_width, normalize, numpy_to_pt, pil_to_numpy, resize,
...
from flashgen.utils import PRECISION_TO_TYPE
...
class Hy15ImageEncodingStage(ImageEncodingStage):
    ...
    def forward(self, batch: ForwardBatch, fastvideo_args: FlashgenArgs) -> ForwardBatch:
        """
        Encode the prompt into image encoder hidden states.
        """
    ...
    def forward(self, batch: ForwardBatch, fastvideo_args: FlashgenArgs) -> ForwardBatch:
        ...
        if batch.pil_image is None:
            ...
            if self.image_encoder is not None:
                ...
                image_np = np.array(resize(image_pil, batch.height, batch.width, resize_mode="crop"))
...
class ImageVAEEncodingStage(PipelineStage):
    ...
    def forward(
        ...
        # Encode Image
        with torch.autocast(device_type=current_platform.device_type, dtype=vae_dtype, enabled=vae_autocast_enabled):
