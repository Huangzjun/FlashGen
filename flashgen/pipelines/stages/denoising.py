...
from flashgen.attention import get_attn_backend
from flashgen.distributed import (get_local_torch_device, get_world_group)
...
from flashgen.models.loader.component_loader import TransformerLoader
...
from flashgen.pipelines.stages.validators import StageValidators as V
from flashgen.pipelines.stages.validators import VerificationResult
from flashgen.platforms import AttentionBackendEnum, current_platform
from flashgen.utils import dict_to_3d_list, masks_like
...
class DenoisingStage(PipelineStage):
    ...
    def forward(
        ...
        # variant, Castillo et al. 2023).  When envs.FLASHGEN_CFG_GATE_STEP
        ...
        _cfg_gate_fraction = envs.FLASHGEN_CFG_GATE_STEP
        if not 0.0 <= _cfg_gate_fraction <= 1.0:
            raise ValueError(f"FLASHGEN_CFG_GATE_STEP must be in [0.0, 1.0], got {_cfg_gate_fraction!r}. "
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                with torch.autocast(device_type=current_platform.device_type, dtype=target_dtype, enabled=autocast_enabled):
                    attn_metadata = None
                    ...
                    if batch.do_classifier_free_guidance:
                        ...
                        if _use_cached_delta:
                            ...
                            # default (FLASHGEN_CFG_GATE_STEP=1.0, _cfg_gate_step_idx
    ...
    def verify_input(self, batch: ForwardBatch, fastvideo_args: FlashgenArgs) -> VerificationResult:
    ...
    def verify_output(self, batch: ForwardBatch, fastvideo_args: FlashgenArgs) -> VerificationResult:
        ...
        with self.progress_bar(total=len(timesteps)) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                with (
                        ...
                        torch.autocast(device_type=current_platform.device_type, dtype=target_dtype, enabled=autocast_enabled),
                ):
    ...
    def forward(
        ...
        with self.progress_bar(total=len(timesteps)) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if hasattr(self, 'interrupt') and self.interrupt:
                    continue
                if not fastvideo_args.use_fsdp_inference:
                ...
                latent_model_input = latents.to(device=transformer_device, dtype=target_dtype)

                if batch.image_latent is not None:
                    latent_model_input = torch.cat(
                        [latent_model_input, batch.image_latent.permute(0, 2, 1, 3, 4).to(transformer_device)],
                ...
                with torch.autocast(device_type=current_platform.device_type, dtype=target_dtype, enabled=autocast_enabled):
                    ...
                    if i < len(timesteps) - 1:
                        ...
                        noise = torch.randn(video_raw_latent_shape,
                                            ...
                                            generator=batch.generator[0]).to(pred_video.device)
