...
from flashgen.layers.quantization.base_config import (
    QuantizationConfig,
    QuantizeMethodBase,
)
...
# yapf: disable
from flashgen.models.parameter import (BasevLLMParameter,
                                        ...
                                        RowvLLMParameter)
# yapf: enable
from flashgen.models.utils import set_weight_attrs
...
class LinearBase(torch.nn.Module):
    ...
    def __init__(
        ...
        if quant_config is None:
            ...
            # has decided not to quantize. Fall back to
            # ``UnquantizedLinearMethod`` so untagged layers behave like a
            # plain ``nn.Linear`` instead of breaking subclass asserts.
...
class ReplicatedLinear(LinearBase):
    ...
    def __init__(
        ...
        # layers it does not intend to quantize. Fall back to
        # ``UnquantizedLinearMethod`` so non-matched layers behave like a
        # plain ``nn.Linear``.
        if self.quant_method is None:
            self.quant_method = UnquantizedLinearMethod()
