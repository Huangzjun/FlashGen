...
Use this file when adding Flashgen-native model components. Keep it generic:
...
- Use `ColumnParallelLinear` and `RowParallelLinear` for tensor-parallel encoder
  ...
  patterns.
- Do not replace a simple official layer with a fused Flashgen layer unless the
...
- Raw `torch.nn.functional.scaled_dot_product_attention` is acceptable for
  unusual cross-modality flat streams when no Flashgen distributed primitive
...
- Prototype the native component before writing conversion mappings. The
  prototype's `state_dict()` is the source of truth for Flashgen target keys and
  shapes.
...
- Fused and packed Flashgen layers may require tensor split/fuse logic in the
