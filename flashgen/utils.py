...
STR_BACKEND_ENV_VAR: str = "FLASHGEN_ATTENTION_BACKEND"
...
def import_pynvml():
    ...
    - `nvidia-ml-py` (https://pypi.org/project/nvidia-ml-py/): The official
        wrapper. It is a dependency of Flashgen, and is installed when users
        install Flashgen. It provides a Python module named `pynvml`.
    ...
    """
    import flashgen.third_party.pynvml as pynvml
    return pynvml
...
def resolve_hf_token() -> str | None:
    ...
    a Flashgen convention; `huggingface_hub` itself doesn't read it).
...
def log_torch_cuda_memory(tag: str | None = None,
    ...
    """Log NPU memory statistics via logger and append to a trace file."""

    log_fn = log_fn or logger.info
    prefix = f"[{tag}] " if tag else ""

    if not hasattr(torch, "npu") or not torch.npu.is_available():
        message = f"{prefix}NPU not available on this host."
        log_fn(message)
    ...
    try:
        ...
        allocated = torch.npu.memory_allocated(device_index)
        reserved = torch.npu.memory_reserved(device_index)
        max_allocated = torch.npu.max_memory_allocated(device_index)
        max_reserved = torch.npu.max_memory_reserved(device_index)
        free_mem, total_mem = torch.npu.mem_get_info(device_index)
    except Exception as exc:  # noqa: BLE001
        message = f"{prefix}Unable to query NPU memory stats: {exc}"
        log_fn(message)
        _append_to_memory_trace(message, log_file_path)
        return
    ...
    message = f"{prefix}NPU memory stats: {' | '.join(stats)}"
    log_fn(message)
    _append_to_memory_trace(message, log_file_path)
...
def get_ip() -> str:
    ...
    warnings.warn(
        ...
        "The value can be set by the environment variable FLASHGEN_HOST_IP.",
        stacklevel=2)
    return "0.0.0.0"
...
def get_loopback_ip() -> str:
    loopback_ip = envs.FLASHGEN_LOOPBACK_IP
    if loopback_ip:
        return loopback_ip

    # FLASHGEN_LOOPBACK_IP is not set, try to get it based on network interface
    ...
    if test_loopback_bind("127.0.0.1", socket.AF_INET):
        ...
        raise RuntimeError("Neither 127.0.0.1 nor ::1 are bound to a local interface. "
                           "Set the FLASHGEN_LOOPBACK_IP environment variable explicitly.")
...
def force_spawn() -> None:
    if os.environ.get("FLASHGEN_WORKER_MULTIPROC_METHOD") == "fork":
        logger.warning("We must use the `spawn` multiprocessing start method.")
        os.environ["FLASHGEN_WORKER_MULTIPROC_METHOD"] = "spawn"


def get_mp_context() -> BaseContext:
    """Get a multiprocessing context with a particular method (spawn or fork).
    By default we follow the value of the FLASHGEN_WORKER_MULTIPROC_METHOD to
    determine the multiprocessing method (default is fork). However, under
    certain conditions, we may enforce spawn and override the value of
    FLASHGEN_WORKER_MULTIPROC_METHOD.
    """
    force_spawn()
    mp_method = envs.FLASHGEN_WORKER_MULTIPROC_METHOD
