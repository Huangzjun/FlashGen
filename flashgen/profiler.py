...
"""Utilities for managing the PyTorch profiler within Flashgen.
...
``FLASHGEN_TORCH_PROFILE_MODEL_LOADING=1``) or via the consolidated
``FLASHGEN_TORCH_PROFILE_REGIONS`` comma-separated list (e.g.
...
def get_or_create_profiler(trace_dir: str | None) -> TorchProfilerController:
    ...
    logger.info(
        ...
        envs.FLASHGEN_TORCH_PROFILER_RECORD_SHAPES,
        envs.FLASHGEN_TORCH_PROFILER_WITH_PROFILE_MEMORY,
        envs.FLASHGEN_TORCH_PROFILER_WITH_STACK,
        envs.FLASHGEN_TORCH_PROFILER_WITH_FLOPS,
    )
    logger.info("FLASHGEN_TORCH_PROFILE_REGIONS=%s", envs.FLASHGEN_TORCH_PROFILE_REGIONS)

    profiler = torch.profiler.profile(
        activities=_DEFAULT_ACTIVITIES,
        record_shapes=envs.FLASHGEN_TORCH_PROFILER_RECORD_SHAPES,
        profile_memory=envs.FLASHGEN_TORCH_PROFILER_WITH_PROFILE_MEMORY,
        with_stack=envs.FLASHGEN_TORCH_PROFILER_WITH_STACK,
        with_flops=envs.FLASHGEN_TORCH_PROFILER_WITH_FLOPS,
        schedule=torch.profiler.schedule(
            wait=envs.FLASHGEN_TORCH_PROFILER_WAIT_STEPS,
            warmup=envs.FLASHGEN_TORCH_PROFILER_WARMUP_STEPS,
            active=envs.FLASHGEN_TORCH_PROFILER_ACTIVE_STEPS,
        ),
...
class TorchProfilerConfig:
    ...
    ``FLASHGEN_TORCH_PROFILE_REGIONS`` environment variable. The resulting
    ...
    def from_env(cls) -> TorchProfilerConfig:
        ...
        requested_regions = {
            token.strip()
            for token in (getattr(envs, "FLASHGEN_TORCH_PROFILE_REGIONS", "") or "").split(",") if token.strip()
        }

        if not requested_regions:
            available = ", ".join(region.name for region in list_profiler_regions())
            raise ValueError("FLASHGEN_TORCH_PROFILE_REGIONS must list at least one region; "
        ...
        if not regions:
            raise ValueError("FLASHGEN_TORCH_PROFILE_REGIONS did not match any known regions; "
...
class TorchProfilerController:
    ...
    def _set_collection(self, enabled: bool) -> None:
        ...
        if self._collection_enabled == enabled:
            return
        event = ("flashgen.profiler.enable_collection" if enabled else "flashgen.profiler.disable_collection")
    ...
    def region(self, region: str):
        ...
        with torch.profiler.record_function(f"flashgen.region::{region}"):
