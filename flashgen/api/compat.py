...
from flashgen.api.overrides import apply_overrides, normalize_overrides
from flashgen.api.parser import config_to_dict, load_raw_config, parse_config
from flashgen.api.request_metadata import (
    EXPLICIT_PATHS_ATTR,
...
from flashgen.utils import shallow_asdict
...
def generator_config_to_fastvideo_args(config: GeneratorConfig | Mapping[str, Any], ) -> FlashgenArgs:
    ...
    if quantization is not None and quantization.transformer_quant is not None:
        ...
        # ``pipeline_config.dit_config.quant_config``; the
    ...
    return FlashgenArgs.from_kwargs(**kwargs)
...
def _compile_config_to_torch_kwargs(compile_config: CompileConfig, ) -> dict[str, Any]:
    ...
    dict that the legacy ``FlashgenArgs`` path still expects.
...
def explicit_request_updates(request: GenerationRequest) -> dict[str, Any]:
    ...
    populated by :func:`flashgen.api.parser.parse_config` or
    :func:`flashgen.api.compat.normalize_generation_request`. Calling on
...
def register_continuation_kind(kind: str) -> None:
    ...
    Per-kind payload deserializers live with the model family that owns
    the continuation kind. The registry lets the public-API compat layer
    validate the kind early, before the state reaches the pipeline.
    """
