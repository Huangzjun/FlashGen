...
(e.g. ``flashgen/pipelines/basic/wan/presets.py``) and registered
explicitly from :func:`_register_presets` in ``flashgen/registry.py``.
"""
...
def get_preset(
    ...
    Raises :class:`~flashgen.api.errors.ConfigValidationError` when the
...
def validate_stage_names(
    ...
    Raises :class:`~flashgen.api.errors.ConfigValidationError` with a
