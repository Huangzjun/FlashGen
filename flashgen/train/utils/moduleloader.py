...
from flashgen.flashgen_args import ExecutionMode, TrainingArgs
from flashgen.models.loader.component_loader import (
    PipelineComponentLoader, )
from flashgen.utils import (
...
# ------------------------------------------------------------------
# TrainingArgs builders (only place that creates FlashGenArgs)
# ------------------------------------------------------------------
...
def load_module_from_path(
    *,
    ...
    """
    flashgen_args: Any = _make_training_args(training_config, model_path=model_path)
    ...
    if override_transformer_cls_name is not None:
        ...
        flashgen_args.override_transformer_cls_name = str(override_transformer_cls_name)

    if transformer_override_safetensor:
        flashgen_args.init_weights_from_safetensors = str(transformer_override_safetensor)

    if disable_custom_init_weights:
        flashgen_args._loading_teacher_critic_model = True
    try:
        ...
        if disable_custom_init_weights and hasattr(flashgen_args, "_loading_teacher_critic_model"):
            del flashgen_args._loading_teacher_critic_model
        if override_transformer_cls_name is not None:
            if old_override is None:
                if hasattr(
                    ...
                    flashgen_args.override_transformer_cls_name = (None)
            else:
                flashgen_args.override_transformer_cls_name = (old_override)
