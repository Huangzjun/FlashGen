...
class CheckpointManager:
    ...
    def _save_rng_snapshot(self, checkpoint_dir: Path) -> None:
        ...
        Each rank saves its own file because NPU RNG and
        ...
        if hasattr(torch, "npu"):
            rng["npu_rng"] = torch.npu.get_rng_state()
        rng["gen_device"] = self.method.cuda_generator.get_state()
        torch.save(
            rng,
    ...
    def load_rng_snapshot(
        ...
        if "npu_rng" in rng and hasattr(torch, "npu"):
            torch.npu.set_rng_state(rng["npu_rng"])
        elif "cuda_rng" in rng and hasattr(torch, "npu"):
            torch.npu.set_rng_state(rng["cuda_rng"])
        gen_state = rng.get("gen_device", rng.get("gen_cuda"))
        if gen_state is not None:
            self.method.cuda_generator.set_state(gen_state)
        logger.info(
