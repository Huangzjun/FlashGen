...
from flashgen.train.utils.checkpoint import _RoleModuleContainer
from flashgen.training.checkpointing_utils import (
    ModelWrapper,
    OptimizerWrapper,
    SchedulerWrapper,
)
...
class TrainingMethod(torch.nn.Module, ABC):
    ...
    A single shared device RNG generator (``cuda_generator``) is
    ...
    # Shared device RNG generator (initialized in on_train_start).
    ...
    def manages_optimization(self) -> bool:
        ...
        stepping. Methods with a custom outer loop can provide a specific
        ``managed_train_step``.
        """
        return False
    ...
    def _infer_attn_kind() -> Literal["dense", "vsa"]:
        """Derive attn_kind from ``FLASHGEN_ATTENTION_BACKEND``."""
