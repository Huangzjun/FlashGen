...
from flashgen.models.schedulers.scheduling_flow_match_euler_discrete import (
    FlowMatchEulerDiscreteScheduler, )
from flashgen.pipelines import TrainingBatch
from flashgen.training.activation_checkpoint import (
    apply_activation_checkpointing, )
from flashgen.training.training_utils import (
...
from flashgen.train.models.base import ModelBase
from flashgen.train.utils.module_state import (
    apply_trainable, )
from flashgen.train.utils.moduleloader import (
    load_module_from_path, )

if TYPE_CHECKING:
    from flashgen.train.utils.training_config import (
        TrainingConfig, )
    from flashgen.train.utils.lora import LoraConfig
...
class WanModel(ModelBase):
    ...
    def __init__(
        ...
        *,
        ...
        self._negative_prompt_checked = False
    ...
    def init_preprocessors(self, training_config: TrainingConfig) -> None:
        ...
        from flashgen.dataset.dataloader.schema import (
            pyarrow_schema_t2v, )
        from flashgen.train.utils.dataloader import (
    ...
    def _init_timestep_mechanics(self) -> None:
        ...
        self.min_timestep = int(float(tc.model.min_timestep_ratio) * self.num_train_timestep)
        self.max_timestep = int(float(tc.model.max_timestep_ratio) * self.num_train_timestep)

    def ensure_negative_conditioning(self) -> None:
        if self.negative_prompt_embeds is not None:
            return
        if self._negative_prompt_checked:
            return

        assert self.training_config is not None
        tc = self.training_config
        self._negative_prompt_checked = True

        embeds_dir = tc.negative_prompt_embeds_path
        if not embeds_dir:
            embeds_dir = os.path.join(tc.model_path, "negative_prompt_embeds")
        ...
        if os.path.isfile(embed_path) and os.path.isfile(mask_path):
            self.negative_prompt_embeds = torch.load(embed_path, map_location=self.device).to(
                device=self.device,
                dtype=self._get_training_dtype(),
            )
            self.negative_prompt_attention_mask = torch.load(mask_path, map_location=self.device).to(
                device=self.device,
                dtype=self._get_training_dtype(),
            )
            logger.info("Loaded pre-encoded negative prompt from %s", embeds_dir)
        else:
            logger.warning("Negative prompt embeddings not found at %s; using zero unconditional fallback.", embeds_dir)
    ...
    def _build_attention_metadata(self, training_batch: TrainingBatch) -> TrainingBatch:
        ...
        backend = envs.FLASHGEN_ATTENTION_BACKEND
        if backend not in (None, "", "TORCH_SDPA"):
            raise ValueError("FlashGen NPU training only supports FLASHGEN_ATTENTION_BACKEND=TORCH_SDPA, "
                             f"got {backend!r}.")
        if tc.vsa_sparsity > 0.0:
            raise ValueError("FlashGen NPU training does not support VSA metadata; set training.vsa.sparsity to 0.")
    ...
    def _prepare_dit_inputs(
        ...
        if (self.negative_prompt_embeds is not None and self.negative_prompt_attention_mask is not None):
            ...
            neg_embeds = torch.zeros_like(training_batch.encoder_hidden_states)
            neg_mask = torch.zeros_like(training_batch.encoder_attention_mask)
