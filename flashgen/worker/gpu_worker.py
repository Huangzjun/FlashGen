...
from flashgen.distributed import (cleanup_dist_env_and_memory, maybe_init_distributed_environment_and_model_parallel)
from flashgen.distributed.parallel_state import get_local_torch_device
from flashgen.fastvideo_args import FlashgenArgs
from flashgen.logger import init_logger
from flashgen.pipelines import ForwardBatch, LoRAPipeline, build_pipeline
...
class Worker:
    ...
    def __init__(self, fastvideo_args: FlashgenArgs, local_rank: int, rank: int, distributed_init_method: str):
    ...
    def init_device(self) -> None:
        ...
        if current_platform.is_npu_alike():
            torch.npu.set_device(self.device)
            self.init_gpu_memory = torch.npu.mem_get_info()[0]
        else:
            raise RuntimeError("Flashgen is NPU-only: current platform is not NPU.")
