...
from flashgen.utils import get_ip
...
try:
    ...
    class RayWorkerWrapper(WorkerWrapperBase):
        """Ray wrapper for flashgen.worker.Worker, allowing Worker to be
...
def _verify_bundles(placement_group: "PlacementGroup", fastvideo_args: FlashgenArgs, device_str: str):
    ...
    if driver_node_id not in node_id_to_bundle:
        raise RuntimeError(f"driver node id {driver_node_id} is not included in a placement "
                           ...
                           "GPUs in a node `{driver_node_id}` before starting an Flashgen engine.")
...
def _wait_until_pg_ready(current_placement_group: "PlacementGroup"):
    ...
    while time.time() - s < PG_WAIT_TIMEOUT:
        ...
        logger.info(
            ...
            " are the same as FLASHGEN_HOST_IP environment variable"
...
def initialize_ray_cluster(
    ...
    if current_placement_group:
        ...
        # Flashgen engine is also a worker to execute model with an accelerator,
        ...
        if current_node_resource.get(device_str, 0) < 1:
            raise ValueError(f"Current node has no {device_str} available. "
                             f"{current_node_resource=}. Flashgen engine cannot start without "
