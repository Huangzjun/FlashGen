...
from flashgen.distributed.parallel_state import get_dp_group, get_tp_group
...
from flashgen.utils import (decorate_logs, get_distributed_init_method, get_exception_traceback, get_loopback_ip,
                             get_mp_context, get_open_port, kill_itself_when_parent_died, force_spawn)
from flashgen.worker.executor import Executor
from flashgen.worker.worker_base import WorkerWrapperBase
...
class MultiprocExecutor(Executor):
    ...
    def execute_forward(self, forward_batch: ForwardBatch, fastvideo_args: FlashgenArgs) -> ForwardBatch:
        ...
        logging_info = None
        logging_info = responses[0]["logging_info"] if envs.FLASHGEN_STAGE_LOGGING else None
    ...
    def execute_streaming_reset(self, forward_batch: ForwardBatch, fastvideo_args: FlashgenArgs) -> dict[str, Any]:
    ...
    def submit_reset(self, forward_batch: ForwardBatch, fastvideo_args: FlashgenArgs) -> None:
        if not self._streaming_enabled:
            ...
            try:
                ...
                if isinstance(method, str):
                    ...
                    if method == 'execute_forward':
                        ...
                        logging_info = None
                        if envs.FLASHGEN_STAGE_LOGGING:
                        ...
                        extra["peak_memory_mb"] = (current_platform.get_current_memory_usage() / (1024 * 1024))
                        self.pipe.send({
                            "output_batch": result,
                            "logging_info": logging_info,
                            "extra": extra,
