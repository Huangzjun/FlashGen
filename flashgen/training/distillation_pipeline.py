...
from flashgen.models.utils import pred_noise_to_pred_video
from flashgen.pipelines import (ComposedPipelineBase, ForwardBatch, TrainingBatch)
from flashgen.platforms import current_platform
from flashgen.training.activation_checkpoint import (apply_activation_checkpointing)
from flashgen.training.training_pipeline import TrainingPipeline
from flashgen.training.training_utils import (EMA_FSDP, clip_grad_norm_while_handling_failing_dtensor_cases,
                                               get_scheduler, load_distillation_checkpoint,
                                               save_distillation_checkpoint, shift_timestep)
from flashgen.utils import (maybe_download_model, set_random_seed, verify_model_config_and_directory)
...
class DistillationPipeline(TrainingPipeline):
    ...
    def post_init(self) -> None:
        super().post_init()
        embeds_dir = self.training_args.negative_prompt_embeds_path
        if not embeds_dir:
            embeds_dir = os.path.join(self.training_args.model_path, "negative_prompt_embeds")
        embed_path = os.path.join(embeds_dir, "negative_prompt_embeds.pt")
        mask_path = os.path.join(embeds_dir, "negative_prompt_attention_mask.pt")
        if os.path.isfile(embed_path) and os.path.isfile(mask_path):
            self.negative_prompt_embeds = torch.load(embed_path, map_location=self.device).to(dtype=torch.bfloat16)
            self.negative_prompt_attention_mask = torch.load(mask_path, map_location=self.device).to(dtype=torch.bfloat16)
            logger.info("Loaded pre-encoded negative prompt: embed shape %s", self.negative_prompt_embeds.shape)
        else:
            logger.warning("Negative prompt embeddings not found at %s; using zeros fallback.", embeds_dir)
    ...
    def initialize_training_pipeline(self, training_args: TrainingArgs):
        ...
        if training_args.warp_denoising_step:  # Warp the denoising step according to the scheduler time shift
            timesteps = torch.cat((self.noise_scheduler.timesteps.cpu(), torch.tensor([0], dtype=torch.float32))).to(get_local_torch_device())
        ...
        try:
            ...
            if self.generator_ema is not None:
                ...
                if ema_model is None:
                    ...
                    from flashgen.training.training_utils import (custom_to_hf_state_dict,
    ...
    def _prepare_dit_inputs(self, training_batch: TrainingBatch) -> TrainingBatch:
        ...
        if getattr(self, "negative_prompt_embeds", None) is not None:
            bsz = training_batch.encoder_hidden_states.shape[0]
            neg_embeds = self.negative_prompt_embeds
            neg_mask = self.negative_prompt_attention_mask
            if neg_embeds.shape[0] != bsz:
                neg_embeds = neg_embeds.expand(bsz, *neg_embeds.shape[1:])
                neg_mask = neg_mask.expand(bsz, *neg_mask.shape[1:])
            unconditional_dict = {
                "encoder_hidden_states": neg_embeds,
                "encoder_attention_mask": neg_mask,
            }
        else:
            # Fallback: use zeros as unconditional embedding (equivalent to
            # empty-string T5 output for CFG).  This allows DMD training to
            # proceed without a pre-computed negative prompt embedding.
            unconditional_dict = {
                "encoder_hidden_states": torch.zeros_like(training_batch.encoder_hidden_states),
                "encoder_attention_mask": torch.zeros_like(training_batch.encoder_attention_mask),
            }
    ...
    def train(self) -> None:
        ...
        self.noise_gen_device = torch.Generator(device=current_platform.device_name).manual_seed(self.seed +
                                                                                                  self.global_rank)
        ...
        for step in range(self.init_steps + 1, self.training_args.max_train_steps + 1):
            ...
            progress_bar.set_postfix({
                ...
                "grad_norm": grad_norm,
                "ema": "on" if (self.generator_ema is not None and self.is_ema_ready()) else "off",
                "ema2": "on" if (self.generator_ema_2 is not None and self.is_ema_ready()) else "off",
            })
        ...
        if envs.FLASHGEN_TORCH_PROFILER_DIR:
