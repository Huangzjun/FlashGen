...
from flashgen.profiler import profile_region
import imageio
import numpy as np
import torch
...
from flashgen.dataset import build_parquet_map_style_dataloader
from flashgen.dataset.dataloader.schema import pyarrow_schema_t2v
...
from flashgen.models.vision_utils import load_video
from flashgen.pipelines import (ComposedPipelineBase, ForwardBatch, LoRAPipeline, TrainingBatch)
...
from flashgen.training.trackers import (DummyTracker, TrackerType, initialize_trackers, Trackers)
from flashgen.training.training_utils import (clip_grad_norm_while_handling_failing_dtensor_cases,
...
from flashgen.utils import set_random_seed, shallow_asdict
...
class TrainingPipeline(LoRAPipeline, ABC):
    ...
    def __init__(self,
                 model_path: str,
                 flashgen_args: TrainingArgs,
                 required_config_modules: list[str] | None = None,
                 loaded_modules: dict[str, torch.nn.Module] | None = None) -> None:
        flashgen_args.inference_mode = False
        self.lora_training = flashgen_args.lora_training
        if self.lora_training and flashgen_args.lora_rank is None:
            raise ValueError("lora rank must be set when using lora training")

        set_random_seed(flashgen_args.seed)  # for lora param init
        super().__init__(model_path, flashgen_args, required_config_modules, loaded_modules)  # type: ignore
    ...
    def initialize_training_pipeline(self, training_args: TrainingArgs):
        ...
        project = training_args.tracker_project_name or "flashgen"
    ...
    def _prepare_dit_inputs(self, training_batch: TrainingBatch) -> TrainingBatch:
        ...
        with self.tracker.timed("timing/prepare_dit_inputs"):
            ...
            noise = torch.randn(latents.shape,
                                generator=self.noise_gen_device,
