...
from flashgen.training.checkpointing_utils import (ModelWrapper, OptimizerWrapper, RandomStateWrapper,
                                                    SchedulerWrapper)
...
def custom_to_hf_state_dict(state_dict: dict[str, Any] | Iterator[tuple[str, torch.Tensor]],
    ...
    """
    Convert flashgen's custom model format to diffusers format using reverse_param_names_mapping.
    
    Args:
        state_dict: State dict in flashgen's custom format
        reverse_param_names_mapping: Reverse mapping from flashgen's custom format to diffusers format
