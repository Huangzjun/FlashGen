...
"""
VideoGenerator module for Flashgen.
...
from flashgen.api.compat import (
...
from flashgen.pipelines import ForwardBatch
from flashgen.utils import align_to, shallow_asdict
from flashgen.worker.executor import Executor
...
class VideoGenerator:
    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        Args:
            ...
            **kwargs: Additional arguments to customize model loading, set any FlashgenArgs or PipelineConfig attributes here.
    ...
    async def generate_async(
        ...
        * Streaming server (:mod:`flashgen.entrypoints.streaming`) —
        ...
        * Dynamo native backend
          (``components/src/dynamo/flashgen/``) — wraps each event as
    ...
    def _generate_video_impl(
        ...
        grid_sizes: tuple[int, int, int] | list[int] | torch.Tensor
        | None = None,
        fastvideo_args: FlashgenArgs | None = None,
        **kwargs,
    ...
    def _save_video_with_audio_ffmpeg_pipe(
        ...
        ffmpeg_bin = shutil.which(os.getenv("FLASHGEN_FFMPEG_BIN", "ffmpeg"))
        ...
        codec = os.getenv("FLASHGEN_VIDEO_CODEC", "libx264")
        ...
        try:
            with tempfile.TemporaryDirectory() as tmpdir:
                ...
                if codec.endswith("_nvenc"):
                    nvenc_options = [
                        ("preset", os.getenv("FLASHGEN_NVENC_PRESET", "p1")),
                        ("tune", os.getenv("FLASHGEN_NVENC_TUNE", "ull")),
                        ("rc", os.getenv("FLASHGEN_NVENC_RC", "constqp")),
                        ("qp", os.getenv("FLASHGEN_NVENC_QP", "28")),
                        ("bf", os.getenv("FLASHGEN_NVENC_BF", "0")),
                    ]
                    ...
                    cmd += ["-preset", os.getenv("FLASHGEN_X264_PRESET", "ultrafast")]

                cmd += [
                    "-c:a",
                    "aac",
                    "-pix_fmt",
                    os.getenv("FLASHGEN_OUTPUT_PIX_FMT", "yuv420p"),
                    "-shortest",
                    "-movflags",
                    "+faststart",
                    output_path,
