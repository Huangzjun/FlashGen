...
"""Zero-overhead-when-off activation trace mode for Flashgen pipelines.

Enable by setting FLASHGEN_TRACE_ACTIVATIONS=1. When off, this module
...
whose name matches FLASHGEN_TRACE_LAYERS, computes the requested stats
(FLASHGEN_TRACE_STATS) on each output tensor, and writes JSONL records
to FLASHGEN_TRACE_OUTPUT.

Useful for parity debugging across model ports — log on both the
Flashgen path and the upstream reference, diff the two JSONL files
to find the first divergent layer.

Example:

    FLASHGEN_TRACE_ACTIVATIONS=1 \
    FLASHGEN_TRACE_LAYERS="^block\\.layers\\.[0-9]+$" \
    FLASHGEN_TRACE_STATS="abs_mean,max,shape" \
    FLASHGEN_TRACE_OUTPUT="/tmp/fv_trace.jsonl" \
...
from flashgen import envs
from flashgen.hooks.hooks import ForwardHook, ModuleHookManager
...
def _resolve_stats(spec: str) -> list[tuple[str, Callable[[torch.Tensor], Any]]]:
    ...
    for name in [s.strip() for s in spec.split(",") if s.strip()]:
        ...
        if stat_fn is None:
            logger.warning(
                "FLASHGEN_TRACE_STATS contains unknown stat %r; valid: %s",
                name,
                sorted(_STAT_FNS),
            )
            continue
...
def attach_activation_trace(model: nn.Module | None) -> ActivationTraceManager | None:
    ...
    if not envs.FLASHGEN_TRACE_ACTIVATIONS or model is None:
        return None

    pattern_spec = envs.FLASHGEN_TRACE_LAYERS
    pattern = re.compile(pattern_spec) if pattern_spec else re.compile(".*")
    stats = _resolve_stats(envs.FLASHGEN_TRACE_STATS)
    if not stats:
        logger.warning("FLASHGEN_TRACE_STATS yielded no valid stats; trace disabled.")
        return None

    sink = JsonlSink(_resolve_output_path(envs.FLASHGEN_TRACE_OUTPUT))
    step_filter = _parse_step_filter(envs.FLASHGEN_TRACE_STEPS)
    managers = []
